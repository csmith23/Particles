...
class NGForce extends Force {
  ...
  calculateForce(Particle[] particles) {
    for(int i = 0; i < particles.length; i++) {
      center_x += particles[i].pos_x;
      center_y += particles[i].pos_y;
  ...
  ApplyForce(Particle particle) {
    ...
    double distance_x = center_x - particles.pos_x;
    double distance_y = center_y - particles.pos_y;
